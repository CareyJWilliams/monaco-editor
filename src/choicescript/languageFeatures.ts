...
function toDiagnostics(resource: Uri, diag: lsTypes.Diagnostic): editor.IMarkerData {
	...
	return {
		severity: diag.severity ? toSeverity(diag.severity) : MarkerSeverity.Info,
			...
			.then((info) => {
				...
				let items: languages.CompletionItem[] = info.items.map((entry) => {
					let item: languages.CompletionItem = {
						...
						detail: entry.detail,
						range: wordRange,
						kind: entry.kind ? toCompletionItemKind(entry.kind) : lsTypes.CompletionItemKind.Value
					};
					if (entry.textEdit) {
					...
					if (info)
						return {
							value: {
								signatures: [
									{
										label: `${sceneName}.txt -> ${routineName}: ${info.parameters
											.map((p) => p.label)
											.join(' ')}`,
...
export class DefinitionAdapter {
	...
	public async provideDefinition(
		...
		const definition = await worker.findDefinition(resource.toString(), fromPosition(position));
		if (definition) return model.isDisposed() ? undefined : [toLocation(definition)];
		return;
	}
}
