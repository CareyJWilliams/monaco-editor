...
var moduleExports = (() => {
  ...
  var choicescriptMode_exports = {};
  __export(choicescriptMode_exports, {
    setupMode: () => setupMode
  });
  ...
  // src/choicescript/workerManager.ts
  "use strict";
  ...
  // src/choicescript/languageFeatures.ts
  var DiagnosticsAdapter = class {
    constructor(_languageId, _worker, defaults) {
      ...
      this._disposables.push(defaults.onDictionaryChange((_) => {
        monaco_editor_core_exports.editor.getModels().forEach((model) => {
          if (model.getLanguageId() === this._languageId) {
  ...
  function toCompletionItemKind(kind) {
    let mItemKind = monaco_editor_core_exports.languages.CompletionItemKind;
    switch (kind) {
      ...
      return this._worker(resource).then((worker) => {
        ...
        let items = info.items.map((entry) => {
          let item = {
            ...
            documentation: entry.documentation ? toMarkdownString(entry.documentation) : void 0,
            detail: entry.detail,
            range: wordRange,
    ...
    provideCodeActions(model, range, context, token) {
      ...
      return this._worker(resource).then((worker) => {
        ...
        for (let m of markers) {
          var wordRange = new monaco_editor_core_exports.Range(m.startLineNumber, m.startColumn, m.endLineNumber, m.endColumn);
          let word = model.getWordAtPosition(new monaco_editor_core_exports.Position(wordRange.startLineNumber, wordRange.startColumn));
          if (!word)
            continue;
  ...
  var SignatureHelpAdapter = class {
    ...
    provideSignatureHelp(model, position, token, context) {
      ...
      return this._worker(resource).then((worker) => {
        ...
        let paramIndex = line.slice(match[1].length).trim().split(" ").filter((e) => e).length;
        return worker.getRoutineInfo(resource.toString(), model.getOffsetAt(position)).then((info) => {
          if (info)
            return {
              value: {
                signatures: [
                  {
                    label: `${match[2]}.txt -> ${match[3]}: ${info.parameters.map((p) => p.label).join(" ")}`,
  ...
  var IndexAdapter = class {
    constructor(_languageId, _worker) {
      ...
      const onModelAdd = (model) => {
        ...
        this._listener[model.uri.toString()] = model.onDidChangeContent(() => {
          window.clearTimeout(handle);
          handle = window.setTimeout(() => this._updateIndex(model.uri), 1e3);
        });
  ...
  function setupMode(defaults) {
    ...
    function registerProviders() {
      ...
      if (true) {
        providers.push(monaco_editor_core_exports.languages.registerSignatureHelpProvider(languageId, new SignatureHelpAdapter(worker)));
      }
      if (modeConfiguration.diagnostics) {
        providers.push(monaco_editor_core_exports.languages.registerCodeActionProvider(languageId, new CodeActionAdapter(worker)));
      }
      if (modeConfiguration.diagnostics) {
      ...
      if (modeConfiguration.autoFormat) {
        ...
        providers.push(monaco_editor_core_exports.languages.registerOnTypeFormattingEditProvider(languageId, {
          ...
          provideOnTypeFormattingEdits: function(model, position, character, options, token) {
            ...
            var range = new monaco_editor_core_exports.Range(position.lineNumber, position.column >= matchLength ? position.column - matchLength : 0, position.lineNumber, position.column);
      ...
      providers.push(new IndexAdapter(languageId, worker));
      disposables.push(monaco_editor_core_exports.editor.registerCommand("add-words", (acc, args) => {
        if (Array.isArray(args)) {
          args = args[0];
        }
        worker(args.uri).then((worker2) => {
          worker2.addWords(args.words).then((results) => {
            defaults.addDictWords(args.dict, results.filter((w) => w));
          });
        });
      }));
      disposables.push(monaco_editor_core_exports.editor.registerCommand("remove-words", (acc, args) => {
        worker(args.uri).then((worker2) => {
          worker2.removeWords(args.words).then((results) => {
            defaults.removeDictWords(args.dict, results.filter((w) => w));
          });
        });
      }));
    }
    worker(new monaco_editor_core_exports.Uri()).then((worker2) => {
      let wordsObj = defaults.diagnosticsOptions.spellcheck.userDictionary;
      let wordsArr = [];
      for (let w in wordsObj) {
        wordsArr.push(w);
      }
      worker2.addWords(wordsArr);
    });
    registerProviders();
  ...
  return choicescriptMode_exports;
})();
